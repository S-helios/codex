...
```rust
// codex-rs/core/src/context_manager/history.rs:52
pub(crate) struct ContextManager {
    ...
    reference_context_item: Option<TurnContextItem>, // baseline 上下文快照（用于 diff 出"设置更新"项）
}
```
...
```rust
// clone_history(): session/mod.rs:2906（取 ContextManager 快照）
let history = sess.clone_history().await;             // ContextManager 快照
// for_prompt(): context_manager/history.rs:145（消费 self，按模型 modalities 产出 Vec<ResponseItem>）
...
Phase 1（提炼）: 一个 LLM（gpt-5.4-mini）读取 rollout，提炼 raw_memory + rollout_summary
    ↓ 存入独立的记忆 DB（memories_1.sqlite，与 state.db 分离）
...
> **重构说明（记忆表已从 state.db 搬出）**：早期版本把 Phase 1 的 `stage1_outputs` 表和流水线 `jobs`
> 表放在主 state DB 里。迁移 `state/migrations/0035_drop_memory_tables.sql` 把这两张表从 state.db
> 删除（`DROP TABLE jobs; DROP TABLE stage1_outputs;`），改由独立的记忆数据库承载——
> 见 `state/memory_migrations/0001_memories.sql`（重建 `stage1_outputs` / `jobs` 两表）。
> 该独立 DB 的文件名常量为 `MEMORIES_DB_FILENAME = "memories_1.sqlite"`（`state/src/lib.rs:83`），
> 落在 `codex_home` 根目录（即 `~/.codex/memories_1.sqlite`，**注意不是** `memories/` 子目录里）。
> 运行时由 `StateRuntime` 在启动时同时打开 state / logs / goals / memories 四个 SQLite（`state/src/runtime.rs:187-229`），
> 并构造 `MemoryStore { pool, state_pool }`（`state/src/runtime/memories.rs:28`）：`pool` 指向记忆 DB（读写 `stage1_outputs` / `jobs`），
> `state_pool` 仅用于读 `threads` 表元数据（按 `memory_mode` 筛选候选 rollout）。
> 因此下文若提到"state DB 里的 stage-1 outputs"，**实际存储已搬到 `memories_1.sqlite`**；
> 仍留在 state.db 的只有 `threads.memory_mode` 这类线程元数据。

### 触发条件

长期记忆流水线由 app-server 在**每次带输入的 Turn 启动**时触发（`app-server/src/request_processors/turn_processor.rs:462`，仅当 `turn_has_input` 为真，调用方写作 `codex_memories_write::start_memories_startup_task(...)`），对应 crate `codex-memories-write` 的 `memories/write/src/start.rs:22`（`pub fn start_memories_startup_task`）。函数内部再用以下条件提前返回（`start.rs:30`）：
...
**做什么**：
1. 调用 `claim_stage1_jobs_for_startup`（`memories/write/src/phase1.rs:166`）从记忆 DB 认领一批待处理 rollout：
   `scan_limit = THREAD_SCAN_LIMIT = 5000`（一次最多扫描的线程数），`max_claimed = max_rollouts_per_startup`（本次真正认领并处理的上限，配置项）
2. 筛选条件（由 `Stage1StartupClaimParams` 传入）：
   - 来自交互式 session 来源（`INTERACTIVE_SESSION_SOURCES`，不是 sub-agent）
   - 在配置的 age window 内（`max_rollout_age_days`）且空闲足够久（`min_rollout_idle_hours`）
   - 线程 `memory_mode = 'enabled'`，且未被其他 worker 认领（job lease 机制）
...
5. 将结果写入记忆 DB 的 `stage1_outputs` 表（独立的 `memories_1.sqlite`，非 state.db；见 §3 重构说明）
...
**做什么**：
1. 认领全局 Phase 2 锁（记忆 DB 的 `jobs` 表，`job_kind = "memory_consolidate_global"`），确保同时只有一个 consolidation
2. 从记忆 DB 加载最新的 `stage1_outputs`（按 usage 排名，最多 N 条；`list_stage1_outputs_for_global` 会跳过 `memory_mode != 'enabled'` 的线程）
3. 同步文件系统 artifacts：
...
6. 将 Phase 2 结果（job 完成状态、watermark 等）写回记忆 DB 的 `jobs` 表
...
**安装位置**：`codex_memories_extension::install(&mut builder, codex_otel::global())`（`app-server/src/extensions.rs:33`，crate 入口 `ext/memories/src/lib.rs:9` 的 `pub use extension::install`）。宿主在组装提示词时遍历所有 context contributor 产出的 `PromptFragment`，按其 `slot()` 分流：`PromptSlot::DeveloperPolicy`（记忆走这条）/ `DeveloperCapabilities` → `developer_sections`；`ContextualUser` → `contextual_user_sections`；`SeparateDeveloper` → `separate_developer_sections`（汇编循环在 `session/mod.rs:2808-2829`，紧邻 AGENTS.md / 环境上下文的注入段；`PromptSlot` / `PromptFragment::developer_policy` 定义在 `ext/extension-api/src/contributors/prompt.rs`）。
...
```rust
// protocol/src/protocol.rs:704-709
...
```rust
// Op 变体定义：protocol/src/protocol.rs:674（字段为 mode: ThreadMemoryMode；字符串映射 "set_thread_memory_mode" 在 :797）
...
这个状态存储在 state DB（SQLite）`threads` 表的 `memory_mode` 列中（该列仍留在 state.db，未随 §3 的记忆 DB 拆分而迁移）。Phase 1 在选择 rollout 时的 SQL 条件是 `threads.memory_mode = 'enabled'`（`state/src/runtime/memories.rs:210`），因此会**同时排除** `"disabled"` 和 `"polluted"` 两种取值的线程。
...
|------|------|------|------|
| **短期记忆** | | | |
| ContextManager 定义 | `core/src/context_manager/history.rs` | 52 | `pub(crate) struct`，`items: Vec<ResponseItem>` |
| 历史 → LLM 输入 | `core/src/context_manager/history.rs` | 145 | `for_prompt(mut self, &[InputModality]) -> Vec<ResponseItem>` |
| 追加历史 | `core/src/session/mod.rs` | 2536 | `record_conversation_items()` |
| 读取历史快照 | `core/src/session/mod.rs` | 2906 | `clone_history()` |
| Compact 操作 | `core/src/session/handlers.rs` | 491 | `compact()` handler |
...
| 注入到 prompt | `core/src/session/mod.rs` | 2830 | `user_instructions` → `UserInstructions.render()`（推入 `contextual_user_sections`） |
| **配置** | | | |
| MemoriesToml / MemoriesConfig | `config/src/types.rs` | 261 / 294 | TOML 形态 / 应用默认后的有效配置 |
| ThreadMemoryMode | `protocol/src/protocol.rs` | 706 | `Enabled` / `Disabled`（serde lowercase） |
| Op::SetThreadMemoryMode | `protocol/src/protocol.rs` | 674 | 设置线程记忆开关（字符串映射 `:797`） |
| 记忆 DB 文件名常量 | `state/src/lib.rs` | 83 | `MEMORIES_DB_FILENAME = "memories_1.sqlite"`（见 §3 重构说明） |
| 记忆 DB 表结构 | `state/memory_migrations/0001_memories.sql` | - | `stage1_outputs` / `jobs` 两表（已从 state.db 移出） |
| state.db 删表迁移 | `state/migrations/0035_drop_memory_tables.sql` | - | `DROP TABLE jobs; DROP TABLE stage1_outputs;` |
| MemoryStore（双 pool） | `state/src/runtime/memories.rs` | 28 | `pool`=记忆 DB / `state_pool`=state.db（读 `threads.memory_mode`） |
