...
├── sessions/             ← 活跃会话；写入端按「本地日期」分桶为 YYYY/MM/DD 子目录
│   └── 2025/05/07/
│       └── rollout-2025-05-07T17-24-21-{uuid}.jsonl  ← 完整对话历史（JSONL，权威来源）
└── archived_sessions/    ← 已归档会话（扁平布局，直接平铺，不再分日期目录）
    └── rollout-{ts}-{uuid}.jsonl
```

> 注意「活跃」与「归档」两套布局不同：写入端 `precompute_log_file_info`
> （`codex-rs/rollout/src/recorder.rs`）把活跃 rollout 落到
> `sessions/YYYY/MM/DD/` 日期子目录；列举活跃会话走目录递归
> （`traverse_directories_for_paths`），同时保留 `traverse_flat_paths` 兼容旧版
> 写出的扁平文件。归档会话则列举为 `ThreadListLayout::Flat`——直接平铺在
> `archived_sessions/` 下。
...
机的完整语义见文档 [`07_goal_mode.md`](../4_工具与多Agent/07_goal_mode.md)。
...
- **活跃会话按本地日期分桶**：写入端 `precompute_log_file_info`
  （`recorder.rs`）用 `OffsetDateTime::now_local()` 取得年 / 月 / 日，拼成
  `sessions/YYYY/MM/DD/` 三级目录（月、日补零两位），再放文件——避免单一目录
  下堆积海量文件拖慢列举。归档会话则**扁平**铺在 `archived_sessions/` 下。
- 文件名中的时间戳用 `-` 而非 `:` 分隔时分秒（`[year]-[month]-[day]T[hour]-[minute]-[second]`，
  规避不允许冒号的文件系统）；末尾 `UUID` 对应 `threads.id`。
- 列举活跃会话走目录递归（`list.rs::traverse_directories_for_paths`），并保留
  `traverse_flat_paths` 兼容旧版本写出的扁平文件。
...
```json
{ "timestamp": "2024-11-15T09:30:45.123Z", "type": "session_meta", "payload": { ... } }
```

`RolloutLine` = `{ timestamp: String, #[serde(flatten)] item: RolloutItem }`。
`timestamp` 为 UTC，毫秒精度（写入端格式 `…T[hour]:[minute]:[second].[subsecond
digits:3]Z`，故 3 位小数——注意这与上面**文件名**里用 `-` 的时间戳不同）；
`type` + `payload` 由 `RolloutItem` 上的 `#[serde(tag = "type", content =
"payload", rename_all = "snake_case")]` 映射。
...
| 活跃会话 JSONL | `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`（日期分桶） | `SESSIONS_SUBDIR = "sessions"` |
| 归档会话 JSONL | `~/.codex/archived_sessions/rollout-*.jsonl`（扁平） | `ARCHIVED_SESSIONS_SUBDIR = "archived_sessions"` |

### 12.2 路径自定义

- **`sqlite_home`**（四个 DB 文件的所在目录）：解析优先级见
  `core/src/config/mod.rs`——① 配置文件 `sqlite_home` 字段（`Config.sqlite_home`）
  ② 环境变量 `CODEX_SQLITE_HOME`（常量 `SQLITE_HOME_ENV` 定义于
  `state/src/lib.rs`；`resolve_sqlite_home_env` 读取，相对路径按 cwd 解析）
  ③ 都未设置时回退到 `$CODEX_HOME`（即 `~/.codex/`）。
