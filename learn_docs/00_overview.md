...
| **TUI 开发** | 00 → 01 → 02 → 06 → 07 |
| **API/协议开发** | 00 → 02 → 04 → 08 |
| **平台/安全工程师** | 00 → 08（沙箱架构图）→ 各平台实现 |
| **AI/Agent 集成** | 00 → 08（MCP 集成图）→ 07（Goal 模式）|
| **core 源码贡献者** | 00 → 02 → 09（精读指南）→ 逐模块深入 |
| **长任务/自治 Agent** | 07_goal_mode.md 重点阅读 |
