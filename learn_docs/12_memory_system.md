...
pub struct ContextManager {
    ...
    history_version: u64,       // 版本号，用于检测历史变更
}
```
...
长期记忆流水线在以下条件满足时，在**每次根 Session 启动**时异步触发（`start.rs:22`）：
...
```
~/.codex/memories/
├── memory_summary.md          ← ★ 每次对话自动注入 system prompt（5000 token 上限）
...
每次 Turn 开始，`build_initial_context`（`session/mod.rs:~2819`）构建 developer instructions：
...
{
    developer_sections.push(memory_prompt);   // 注入 developer message
}
```

`build_memory_tool_developer_instructions` 的实现（`memories/read/src/prompts.rs:28`）：

```rust
pub async fn build_memory_tool_developer_instructions(codex_home) -> Option<String> {
    ...
    // 截断到 5000 tokens（防止过长）
    let memory_summary = truncate_text(&memory_summary, Tokens(5_000));
    // 渲染 read_path.md 模板
...
3. **快速记忆检索流程**（4-6 步以内）
4. **memory_summary.md 的内容**（实际的记忆摘要，5000 token 上限）
...
```toml
[memories]
# 是否为新线程生成记忆（Phase 1 + Phase 2 流水线）
generate_memories = true   # 默认 true

# 是否将记忆注入到每次对话的 system prompt
use_memories = true        # 默认 true

# Phase 2 整合时最多保留多少条 raw_memory
max_raw_memories_for_consolidation = 200  # 默认值

# 最多多少天未使用的记忆被排除在整合之外
max_unused_days = 90       # 默认值

# 若使用了 MCP 工具或 Web Search，标记该线程为 "polluted"（不生成记忆）
no_memories_if_mcp_or_web_search = true  # 默认 true
```
...
```rust
// submission_loop 中的处理：
Op::SetThreadMemoryMode { mode } => {
    // 仅更新 state DB 中的 memory_mode 元数据
    // 不触发任何 LLM 调用
    handlers::persist_thread_memory_mode_update(&sess, mode).await;
}
```

这个状态存储在 state DB（SQLite）的线程元数据中。Phase 1 在选择 rollout 时会跳过 `memory_mode = "disabled"` 的线程。

**污染标记**（`memory_mode = "polluted"`）：若线程使用了 MCP 工具或 Web Search（且开启了 `no_memories_if_mcp_or_web_search`），自动标记为 polluted，不生成长期记忆。这是为了防止外部动态内容污染记忆（如今天的天气、临时 API 输出）。
...
|------|------|------|------|
| **短期记忆** | | | |
| ContextManager 定义 | `core/src/context_manager/history.rs` | 34 | `items: Vec<ResponseItem>` |
| 追加历史 | `core/src/session/mod.rs` | 2603 | `record_conversation_items()` |
| 读取历史 → LLM | `core/src/session/mod.rs` | 2983 | `clone_history().for_prompt()` |
| Compact 操作 | `core/src/session/handlers.rs` | ~ | `compact()` handler |
| **长期记忆** | | | |
| 启动记忆流水线 | `memories/write/src/start.rs` | 22 | `start_memories_startup_task()` |
...
| 记忆注入 session | `core/src/session/mod.rs` | 2820 | `build_memory_tool_developer_instructions()` |
| 读取 memory_summary | `memories/read/src/prompts.rs` | 28 | 读文件 + 截断到 5000 token |
| 注入模板 | `memories/read/templates/memories/read_path.md` | - | 完整的记忆使用指令模板 |
| **AGENTS.md** | | | |
| AGENTS.md 管理器 | `core/src/agents_md.rs` | 47 | `AgentsMdManager` |
| 发现文件路径 | `core/src/agents_md.rs` | 213 | `agents_md_paths()` 向上遍历 |
| 拼接内容 | `core/src/agents_md.rs` | 82 | `user_instructions()` |
| 全局 AGENTS.md | `core/src/agents_md.rs` | 62 | `load_global_instructions()` |
| 注入到 prompt | `core/src/session/mod.rs` | 2911 | `user_instructions` → user message |
| **配置** | | | |
| MemoriesConfig | `config/src/types.rs` | 291 | `generate_memories`, `use_memories`, etc. |
| ThreadMemoryMode | `protocol/src/protocol.rs` | 770 | `Enabled` / `Disabled` |
| Op::SetThreadMemoryMode | `protocol/src/protocol.rs` | 736 | 设置线程记忆开关 |
