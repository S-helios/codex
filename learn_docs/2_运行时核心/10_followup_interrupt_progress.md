...
```rust
// session/mod.rs:516  ── Codex::spawn_internal（fn 本身在 :487）中创建
let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY); // A：前端 → Session（常量 = 512，定义在 :453）
...
```rust
// Codex::submit_with_id（session/mod.rs:709）
...
**submission_loop 的消费**（handlers.rs:744）：
...
```rust
// Codex::next_event（session/mod.rs:742）
...
```rust
// 审批请求的工程模式（session/mod.rs，oneshot::channel 用法见 2004 一带，下为示意）
...
`Codex::spawn` 最终调用 `tokio::spawn` 启动 `submission_loop`（session/mod.rs:671）：
...
Turn 内部是一个**无限 loop**，每轮对应一次 LLM API 调用（`session/turn.rs:240`，`run_turn` 入口在 :154）：
...
每次采样前构造 prompt（`session/turn.rs:256`）：
...
pub(crate) async fn record_conversation_items(
    ...
    // 层1：append 到内存历史（持 state 锁后调用 state.record_items）
    self.state.lock().await
        .record_items(items.iter(), turn_context.truncation_policy);
    self.persist_rollout_response_items(items).await;        // 层2：持久化到 rollout 文件
    self.send_raw_response_items(turn_context, items).await; // 层3：发事件给前端
}
```
...
steer_input("换个方向")
    ...
    └─ push 到 pending_input（暂存，非阻塞）
                ...
                │ 等待当前 LLM 调用 / 工具执行完毕
                ↓
    loop 顶部：input_queue.get_pending_input()
                │
...
```rust
// state/turn.rs:115 —— pending_input 现在是 TurnInputQueue（不再是裸 Vec）
pub(crate) struct TurnState {
    pending_input: TurnInputQueue,            // Turn 本地暂存队列（内部 items: Vec<TurnInput>）
    mailbox_delivery_phase: MailboxDeliveryPhase,
    // 其他：审批/工具回调的 oneshot::Sender 登记处...
}

// session/input_queue.rs:13 —— 暂存元素的多态枚举
pub(crate) enum TurnInput {
    UserInput(Vec<UserInput>),   // 用户追问（可多模态）
    ResponseItem(ResponseItem),  // 直接注入的 ResponseItem（additional_context / mailbox 投递等）
}

// 真正的存取逻辑搬到了 session/input_queue.rs 的 InputQueue（Session 持有一个实例，
// 同时负责 inter-agent「邮箱」投递的协调）：
//   push：input_queue.extend_pending_input_for_turn_state(turn_state, items)
//         steer 路径走 ..._and_accept_mailbox_delivery_for_turn_state，顺带接受邮箱投递
//   take：input_queue.get_pending_input(&active_turn)
//         内部 turn_state.pending_input.items.split_off(0) 原地取空，
//         再按需 drain 邮箱（inter-agent 通信）一并返回 Vec<TurnInput>
```

### 3.3 loop 顶部的消费逻辑（session/turn.rs:240）

```rust
// 控制何时允许排空 pending_input（turn.rs:194）：
...
loop {  // turn.rs:240
    let pending_input = if can_drain_pending_input {
        // split_off(0) 取空 turn-local 队列 + 按需 drain 邮箱
        sess.input_queue.get_pending_input(&sess.active_turn).await
    } else {
        Vec::new()
    };

    // pending_input 过 hook 检查并写入历史（内部对每条调用
    // record_pending_input → record_conversation_items + 发 UserMessage 事件，
    // 前端因此看到追问消息气泡）；返回 true 表示 hook 要求中止本轮
    ...
    run_sampling_request(/* ... */, cancellation_token.child_token()).await;
...
### 3.5 接口层（codex_thread.rs:269）

```rust
pub async fn steer_input(
    &self,
    input: Vec<UserInput>,
    additional_context: BTreeMap<String, AdditionalContextEntry>, // 追加上下文，与 input 一起入队
    expected_turn_id: Option<&str>,     // 乐观并发控制：防止追问错 Turn
    responsesapi_client_metadata: Option<HashMap<String, String>>,
) -> Result<String, SteerInputError> {
    self.codex
        .steer_input(input, additional_context, expected_turn_id, ...)
        .await
}
```

`Session::steer_input` 的校验流程（session/mod.rs:3151，注意顺序——空输入是最后一道）：

```
1. 无活跃 Turn / 无活跃 task → NoActiveTurn
2. expected_turn_id 与当前 Turn 不匹配 → ExpectedTurnMismatch（乐观锁）
3. Turn 类型为 Review/Compact → ActiveTurnNotSteerable
4. 输入为空 → EmptyInput
5. 合并 additional_context、（可选）写入 responsesapi metadata
6. 通过 → extend_pending_input_and_accept_mailbox_delivery_for_turn_state
         （additional_context 转 ResponseItem，连同 UserInput 一起入队，并接受邮箱投递）
```
...
```rust
// session/turn.rs:265（child_token 实参在 :273）
...
### 4.5 持久化处理（thread_manager.rs:1566，`append_interrupted_boundary`）
...
AgentStatus 通过 `watch::Receiver<AgentStatus>` 多播，多个消费者可同时订阅（`codex_thread.rs:388`）。
...
### 5.3 完整 EventMsg 枚举（protocol.rs:1219）
...
**方式一：next_event() 完整事件流**（codex_thread.rs:380）
...
**方式二：subscribe_status() 粗粒度状态订阅**（codex_thread.rs:388）
...
| `codex-rs/core/src/state/turn.rs` | `TurnState`（`pending_input: TurnInputQueue`）、`ActiveTurn`、`MailboxDeliveryPhase` |
| `codex-rs/core/src/session/input_queue.rs` | `InputQueue`、`TurnInput`、`get_pending_input`、inter-agent 邮箱投递协调 |
| `codex-rs/core/src/context_manager/history.rs` | `ContextManager`（对话历史 Vec）、`for_prompt` |
| `codex-rs/core/src/session/mod.rs` | `Session::steer_input`、`send_event_raw`、`record_conversation_items` |
...
| 位置 | 说明 |
|------|------|
| `session/turn.rs:154` | `run_turn` 入口 |
| `session/turn.rs:240` | 采样主循环 `loop {` |
| `session/turn.rs:244` | `pending_input` 排空（`input_queue.get_pending_input`） |
| `session/turn.rs:256` | `clone_history().for_prompt()` |
| `session/turn.rs:265` | `run_sampling_request` 调用（传 `child_token()`） |
| `session/turn.rs:283` | `has_pending_input` 判断 |
| `state/turn.rs:115` | `TurnState` 结构体（`pending_input: TurnInputQueue`） |
| `session/input_queue.rs:25` | `InputQueue`（会话级暂存 + 邮箱协调） |
| `session/input_queue.rs:158` | `take_pending_input_for_turn_state`（`split_off(0)`） |
| `session/input_queue.rs:169` | `get_pending_input`（取空 pending + drain 邮箱） |
| `context_manager/history.rs:52` | `ContextManager` 结构体 |
| `context_manager/history.rs:145` | `for_prompt` |
| `session/mod.rs:453` | `SUBMISSION_CHANNEL_CAPACITY = 512` |
| `session/mod.rs:2536` | `record_conversation_items` |
| `session/mod.rs:3151` | `Session::steer_input` |
| `session/mod.rs:751` | `Codex::steer_input`（门面转发） |
| `protocol.rs:529` | `pub enum Op` |
| `protocol.rs:1219` | `pub enum EventMsg` |
| `protocol.rs:1632` | `pub enum AgentStatus` |
| `protocol.rs:3707` | `pub enum TurnAbortReason` |
| `codex_thread.rs:269` | `steer_input` |
| `codex_thread.rs:380` | `next_event` |
| `codex_thread.rs:388` | `subscribe_status` |
...
（定义在 `codex-rs/protocol/src/user_input.rs:15`）：

```rust
pub enum UserInput {
    Text { text: String, text_elements: Vec<TextElement> }, // 文本 + UI 富文本标记跨度
    Image { image_url: String, detail: Option<ImageDetail> }, // 预编码 data: URI 图片
    LocalImage { path: PathBuf, detail: Option<ImageDetail> }, // 本地图片路径（序列化时转 Image）
    Skill { name: String, path: PathBuf },                  // 用户选中的 Skill（指向 SKILL.md）
    Mention { name: String, path: String },                 // 结构化 @mention（app:// / plugin://）
}
```
