...
│  └──────────────────────────────────────────────────────────┘   │
│  [当前可切换为：ApprovalOverlay / ListSelectionView /            │
│   RequestUserInputOverlay / McpServerElicitationOverlay /        │
│   FileSearchPopup / CommandPopup / FeedbackNoteView ...]         │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
| **HistoryCell 列表** | `tui/src/history_cell/` | 对话历史中的已完成消息气泡（committed_cells） |
...
```
tui/src/app/
├── agent_message_consolidation.rs — 流式 agent 消息定稿后合并进 scrollback
├── agent_navigation.rs       — 多 agent 导航状态（主/子 agent 切换）
├── app_server_event_targets.rs — app-server 请求/通知的 thread 目标定位
├── app_server_events.rs      — app-server 事件流处理（ServerNotification）
...
├── pets.rs                   — 终端「宠物」彩蛋事件处理
├── platform_actions.rs       — 平台相关操作（打开外部编辑器等）
├── plugin_mentions.rs        — @plugin mention 能力增强（来自 app-server plugin/list）
...
└── thread_settings.rs        — TUI 本地与 app-server 的 thread 设置同步
```
...
└─────────────────────────────────────────────────────────────────────────┘
   ...
   粘贴事件 / 终端 Resize             (ServerNotification / ServerRequest)
          │                                    │
          ▼                                    │
  EventBroker（tui/src/tui/event_stream.rs）  │
  · 过滤 focus/paste 协议事件                  │
...
`HistoryCell`（`tui/src/history_cell/`）是所有气泡的公共 trait，每种气泡实现不同的 `display_lines()` 方法：
...
> 上表为代表性选摘：`chatwidget/` 目录实际包含约 54 个子模块（不含测试文件），
> 此处仅列出与 UI 视图/交互最相关的若干；完整清单以源码目录为准。
...
AI 调用 request_user_input   ───→  RequestUserInputOverlay
                                   · 展示 AI 提问 · 等待用户回答

MCP 服务发起 elicitation     ───→  McpServerElicitationOverlay
                                   · MCP 表单字段 · 用户填写并提交

/model 命令 / OpenAgentPicker ──→  ListSelectionView（列表选择）
                                   · 模型选择 · 沙箱策略 · 审批策略

触发反馈                     ───→  FeedbackNoteView（反馈提交）
打开 App Link                ───→  AppLinkView（应用链接详情）
/experimental                ───→  ExperimentalFeaturesView
/keymap                      ───→  open_keymap_action_menu（动作菜单）/ KeymapCaptureView（按键捕获）
```
...
trait Renderable {
    ...
    fn cursor_style(&self, _area: Rect) -> SetCursorStyle { SetCursorStyle::DefaultUserShape }
}
```
...
codex-ansi-escape crate（codex-rs/ansi-escape/）
  ...
  · 将字节流转换为带样式的文本片段
        ...
        │
        ▼（底层使用 ansi-to-tui 库的 IntoText::into_text）
ansi_escape() -> Text<'static>  /  ansi_escape_line() -> Line<'static>
  · 输出：ratatui::text::Text / Line（含带样式的 Span）
...
| `tui/src/chatwidget.rs` | ChatWidget 主文件（约 1957 行；其余逻辑拆入 chatwidget/）|
| `tui/src/chatwidget/` | ChatWidget 子模块目录（约 54 个模块）|
| `tui/src/history_cell/` | 所有 HistoryCell 类型实现（模块目录）|
