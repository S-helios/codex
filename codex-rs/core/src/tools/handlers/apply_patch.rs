//! `apply_patch` 工具处理器：把模型产出的自由格式补丁校验后落到目标环境文件系统。
//!
//! 【文件职责】定义 [`ApplyPatchHandler`]（实现 `ToolExecutor` / `CoreToolRuntime`）：
//!   解析补丁文本 → 选定目标环境 → 在该环境文件系统上**先校验**（路径合法、上下文匹配）
//!   → 计算所需文件系统权限并按需走审批 → 实际写盘并发 `PatchApplyUpdated` 事件。
//!   另含 [`intercept_apply_patch`]：在 shell 命令里识别「其实是 apply_patch」的调用并改走此路径。
//!
//! 【架构位置】tools 层的写文件工具，是与 `apply-patch` crate（`codex_apply_patch`，纯
//!   解析 / 校验 / 应用逻辑）的对接点。审批经 orchestrator / `request_patch_approval`，
//!   写盘经选定环境的 `ExecutorFileSystem`。
//!
//! 【关键设计】「先校验再应用」+「按 diff 计算最小所需权限」：补丁只在通过沙箱校验后才落盘，
//!   且仅申请本次改动实际涉及的路径权限，避免过度授权。
...
impl ToolExecutor<ToolInvocation> for ApplyPatchHandler {
    ...
    /// 工具调用入口：把一次 `apply_patch` 调用从「文本补丁」走到「落盘」。
    /// 流程：取出 Custom payload 里的补丁文本 → `parse_patch` 解析（失败回 `RespondToModel`
    /// 让模型重写）→ 定位目标环境（多环境时必须显式指定 environment_id）→ 在该环境文件系统上
    /// `verify_apply_patch_args` 校验 → 据校验出的改动集计算权限并应用、发 `PatchApplyUpdated`。
    /// 所有失败都以 `RespondToModel` 返回，让模型看到原因自行纠偏，而非中断回合。
    async fn handle(
        &self,
...
/// shell 路径上的「apply_patch 拦截」：当模型用 shell 工具执行的命令其实是一个 apply_patch
/// 补丁时（`maybe_parse_apply_patch_verified` 能解析并校验通过），就**不真的 spawn 子进程**，
/// 改走与 `ApplyPatchHandler::handle` 相同的「计算权限 → 应用 / 委托运行时」路径。
/// 返回 `Ok(None)` 表示「这不是补丁、按普通命令继续」；`Ok(Some(..))` 表示已作为补丁处理。
/// 这样无论模型走 apply_patch 工具还是塞进 shell，落盘语义与审批都一致。
