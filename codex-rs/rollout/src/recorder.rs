...
//!
//! 把整段会话「逐行 JSONL」落盘（活跃会话按本地日期分桶：
//! `~/.codex/sessions/YYYY/MM/DD/rollout-<时间戳>-<id>.jsonl`；归档后扁平铺在
//! `~/.codex/archived_sessions/` 下），供日后回放、恢复（Resume）或检视（`jq`/`fx`）。
//!
//! 【文件职责】实现 [`RolloutRecorder`]——会话持久化的对外句柄。它本身不碰文件，而是把写入
//!   请求作为 [`RolloutCmd`] 发给一个独占文件句柄的「后台写手任务」（[`rollout_writer`]）。
//!   还提供线程列举/归档/恢复（`list_threads*` / `load_rollout_items` / `find_latest_thread_path`）。
//!
//! 【架构位置】core 侧 `session` 在 `record_conversation_items` 里调
//!   [`RolloutRecorder::record_canonical_items`] 把对话项异步落盘，与「内存 history + 通知 UI」
//!   并列。是会话状态走向磁盘的唯一出口；恢复会话时反向由此读回 `InitialHistory`。
//!
//! 【关键设计】actor 模型 + 有界 mpsc 通道（容量 256）：所有磁盘 I/O 都挪到后台 tokio 任务，
//!   绝不在调用方线程做「阻塞」I/O；通道满时发送 future 让出（yield）而非阻塞，形成自然背压。
//!   写手任务的「终态失败」记在 [`RolloutWriterTask::terminal_failure`]，让后续发送能察觉写手已死
//!   并据此报错——普通 I/O 错误则留在任务内、经命令 ack 回报且把条目留存待重试，不算终态。
...
/// ```
/// 可克隆的轻量句柄：`tx` 是发往后台写手的命令通道；`writer_task` 共享写手任务的可观测状态
/// （句柄 + 终态失败），让任意克隆都能感知写手是否已死；`rollout_path` 是本次 rollout 的文件路径。
#[derive(Clone)]
...
/// 创建 recorder 的两种模式：`Create` 新开会话（写一行 `SessionMeta` 头，含 id/cwd/版本/
/// 来源/动态工具等元信息），`Resume` 续写已有 rollout 文件（以 append 方式打开，不再写头）。
#[derive(Clone)]
...
/// 发给后台写手的命令。`AddItems` 是「即发即忘」追加（不带 ack）；`Persist`/`Flush`/`Shutdown`
/// 各带一个 oneshot `ack`，让调用方能 await 到「确已落盘 / 已冲刷 / 已收尾」。Shutdown 处理成功后
/// 写手跳出循环结束任务。
enum RolloutCmd {
...
impl RolloutRecorder {
    ...
    /// 构造 recorder：据 `Create`/`Resume` 备好文件与（可选）`SessionMeta` 头，建一条容量 256 的
    /// 有界 mpsc，spawn 后台 [`rollout_writer`] 独占文件做异步写，最后返回持有发送端的句柄。
    /// 写手任务若以 Err 收场（终态失败），通过 `mark_failed` 记到共享状态里，供后续发送察觉。
    pub async fn new(
        config: &impl RolloutConfigView,
...
/// 后台写手任务的主循环（actor 本体）：独占文件句柄，从通道里逐条取命令并施加到
/// [`RolloutWriterState`]——`AddItems` 追加并按需冲刷、`Persist`/`Flush` 落盘后回 ack、
/// `Shutdown` 收尾后回 ack 并跳出循环。发送端全部 drop 导致通道关闭时循环也自然结束。
/// 返回的 `Err` 才是终态失败（见文件头【关键设计】）。
